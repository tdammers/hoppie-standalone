...
parseMessage :: Map MessageTypeID MessageType -> ByteString -> [[(MessageTypeID, [ByteString])]]
parseMessage msgTypes = go . scrub
  where
    go remainder = do
      parser <- parsers
    ...
    parsers = flip map (orderBySpecificity $ Map.toList msgTypes) $ \(msgID, msg) -> do
...
downlinkMessages = Map.fromList
    ...
    , ( "HPPD-1", defMessageType { msgPattern = "REQUEST LOGON" } )
    ]
...
runSmokeTests = do
  ...
  forM_ messages $ \msg -> do
    let matching = parseMessage allMessageTypes msg
